...
const CustomerOrder = () => {
  ...
  useEffect(() => {
    const orderMessages = messages.filter(msg => msg.type === 'order-update' && msg.status);
    if (orderMessages.length > 0) {
      setNotifications(orderMessages);
    }
  }, [messages]);
          ...
          {popularDishes.map((dish) => (
            <div key={dish.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="p-4">
                ...
                <p className="text-gray-500 text-sm mt-1">{dish.description || 'Delicious dish'}</p>
                <div className="flex justify-between items-center mt-3">
                  <span className="text-gray-500 text-xs">Orders: {dish.numberOfOrders}</span>
                </div>
              </div>
            </div>
          ))}
